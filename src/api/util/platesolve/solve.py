...
from src.api.util import arcSecondsPerPixel, logCommand, catalog, catalogList, getCoordsFromCatalog
...
def buildSolveCommand(src, dest, scale=None, ra=None, dec=None, radius=None, cpulimit=None, downsample=2, noplots=False):
    ...
    plate_solve_cmd = ['solve-field', "--cpulimit=30",
                       "--crpix-center", "-D", dest, "--overwrite"]
    ...
    if scale == 'safe':
        focal_scale = ["--scale-units", "degwidth",
                       "--scale-low", "0.1", "--scale-high", "180.0"]
        plate_solve_cmd = plate_solve_cmd + focal_scale

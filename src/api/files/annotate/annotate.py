...
from src.api.util import returnResponse, returnFile, buildImageHeaders, buildAnnotatedImage
...
async def annotate_image(image):
    ...
    try:
        image_url = buildAnnotatedImage(
            current_app.config['BASE_IMAGE_DIRECTORY'] + image, light)

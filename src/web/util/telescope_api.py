...
from .telescope import deep_update_telescope_file, TelescopeControlMethods, TelescopeTargetTypes
...
def controlTelescope(method, target):
    if (method not in [x.value for x in TelescopeControlMethods]):
        raise Exception(f'Telescope control method [{method}] not valid. Options: {
                        [x.value for x in TelescopeControlMethods]}')

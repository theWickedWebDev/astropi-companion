...
import json
import os
from api.util import returnResponse, capturePhoto, appendExif, buildImageHeaders, returnFile, buildAnnotatedImage, solveField, logError, generateCameraConfigDict, dict_compare
...
async def capture_single_frame(format="manual"):
    ...
    if (not session.get('active_capture_session')):
        return await returnResponse({"res": "no active session has been set"}, 404)


    camera = trio.CancelScope()
    with camera:
        try:
            return await returnResponse({"res": "captured photo"}, 200)
...
async def cancel_camera_operation():
    ...
    try:
        ...
        logError("cancel_camera_operation", "Cancelling Camera", e)
